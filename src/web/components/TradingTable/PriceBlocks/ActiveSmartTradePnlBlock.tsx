...
const PnlBlock = ({
	...
	let profitPercentage = (currentPrice / entryOrderPrice * 100 - 100) * leverage * (isBuyTypeOrder(side) ? 1 : -1);
	let profitAmount = positionAmount / leverage * entryOrderPrice * (profitPercentage / 100);
	...
	return (
		<SubColumnValue theme={theme} color={+profitPercentage > 0 || templatePnl > 0 ? green.main : red.main}>
			{' '}
			{!!templatePnl ? (
				...
				)} ${pairArr[1]} / ${profitPercentage < 0 ? '-' : ''}${Math.abs(Number(profitPercentage.toFixed(2)))}%`
			)}
		</SubColumnValue>
	);
};
