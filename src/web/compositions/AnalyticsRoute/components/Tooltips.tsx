...
export const PopupInfoBlock = styled.div`
  border-right: ${(props: { isFirstBlock: boolean, theme: Theme }) => props.isFirstBlock && props.theme.palette.border.new};
...
export const TooltipForButterflyChart = ({ id, theme, needQuoteInLabel }: { id: string, theme: Theme, needQuoteInLabel: boolean }) => {
  return ReactDOM.createPortal(
    <SquarePopup theme={theme} id={`butterflyChart-tooltip-${id}`}>
      <PopupDateContainer theme={theme} id={`butterflyChart-tooltip-${id}-date`}>
        1 Jan
      </PopupDateContainer>
      <PopupInfoContainer>
        ...
        <PopupInfoValue id={`butterflyChart-tooltip-${id}-sell`}>0</PopupInfoValue>
          <PopupInfoTitle style={{ color: '#F26D68' }}>{needQuoteInLabel ? 'Avg. Sell' : 'Sell Trades'}</PopupInfoTitle>
        </PopupInfoBlock>
      </PopupInfoContainer>
    </SquarePopup>,
    document.body
