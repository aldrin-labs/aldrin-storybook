...
export const getPriceForMarketOrderBasedOnOrderbook = (orderAmount: number, byType: 'sell' | 'buy', orderbook: { asks: number[][], bids: number[][] }) => {
  const arrayOfOrdersInOrderbook = byType === 'sell' ? orderbook.bids : orderbook.asks
  let priceForOrder = arrayOfOrdersInOrderbook[arrayOfOrdersInOrderbook.length - 1][0]
  const EXPECTED_REACHED_LIMIT = 5
  let alreadyReachedVolumeLimit = 0

  console.log('priceForOrder before: ', priceForOrder)
  ...
  console.log('priceForOrder after: ', priceForOrder)
