...
const BorrowLending: FC = ({ match }: BorrowLendingProps) => {
  ...
  const [walletAccounts, setWalletAccounts] = useState<
    WalletAccountsType | [] | null
  >(null)
  const [walletReserveMap, setWalletReserveMap] = useState<
    WalletReserveMapType | []
  >([])
  const [userSummary, setUserSummary] = useState(null)
  const [reservesRefreshCount, setReservesRefreshCount] = useState(0)
  const [obligationDetails, setObligationDetails] =
  ...
  const handleGetReservesAccounts = () => {
    getReserves({
      ...
      .then((reservesAccounts) => {
        const tempReservesPKs = reservesAccounts.map(
        ...
        Promise.all(promiseArr)
          ...
          .catch((promiseArrError) =>
            console.log('promiseArrError', promiseArrError)
          )
      })
      .catch((error) => {
        console.log('error')
  ...
  const loadObligationFromWallet = (walletPubKey: PublicKey) => {
    ...
    loadAccountsFromProgram({
      ...
      .then((res) => {
        const resCopy = [...res]
        resCopy.sort((a, b) =>
          a.pubkey.toString() > b.pubkey.toString() ? 1 : -1
        )
