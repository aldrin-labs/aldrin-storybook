...
const ActionsPopup = ({
  ...
  const ballanceAfterDeposit = removeTrailingZeros(
    (+walletBalance - amount).toFixed(tokenDecimals)
  )
  const depositAfterDeposit = removeTrailingZeros(
    (+depositAmount + amount).toFixed(tokenDecimals)
  )
  const depositWorthAfterDeposit = depositAfterDeposit * tokenPrice

  const collateralWorthAfterDeposit =
    collateralWorth +
    calcCollateralWorth(
      amount,
      reserveBorrowedLiquidity,
  ...
  const ballanceAfterWithdraw = removeTrailingZeros(
    (+walletBalance + amount).toFixed(tokenDecimals)
  )
  const depositAfterWithdraw = removeTrailingZeros(
    (+depositAmount - amount).toFixed(tokenDecimals)
  )
  const collateralWorthAfterWithdraw =
    collateralWorth -
    calcCollateralWorth(
      amount,
      reserveBorrowedLiquidity,
  ...
  const remainingBorrowAfterDeposit =
    (reserve.config.loanToValueRatio.percent / 100) *
    collateralWorthAfterDeposit
  const remainingBorrowAfterWithdraw =
    (reserve.config.loanToValueRatio.percent / 100) *
    collateralWorthAfterWithdraw
  ...
  console.log(
    'reserveObligation?.collateral.inner.depositedAmount',
            ...
            <RowContainer>
              ...
              <Cell col={6}>
                <BlockSupply>
                  <TitleBlock>Wallet Balance</TitleBlock>
                  {mode
                    ? ballanceAfterWithdraw < 0
                      ? 0
                      : ballanceAfterWithdraw
                    : ballanceAfterDeposit < 0
                    ? 0
                    : ballanceAfterDeposit}
                  <a
                    href={`https://explorer.solana.com/address/${reserve.liquidity.mint.toString()}`}
                    target="_blank"
                    style={{ display: 'block' }}
                ...
                <ButtonGroup>
                  ...
                  <BlueButton
                    ...
                    disabled={
                      !!(
                        amount === 0 ||
                        typeof reserveObligation?.collateral.inner
